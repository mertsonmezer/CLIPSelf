...
class TimmModel(nn.Module):
  ...
  def _extract_roi_features_v1(self, x, normed_boxes, **kwargs):
    ...
    x = roi_align(x, self._denormalize_boxes(normed_boxes, x), (tar_h, tar_w), 1.0, -1, True)
  ...
  def encode_dense(self, x, **kwargs):
    ...
    # Handle ViT output which is 3D [bs, seq_len, embed_dim]
    if x.dim() == 3:
      ...
      feat_h = h // patch_size[0]
      feat_w = w // patch_size[1]

      # Remove CLS token if present (first token)
      if x.shape[1] == feat_h * feat_w + 1:
        x = x[:, 1:]  # Remove CLS token

      # Apply head processing
      x = self.head(x)

      # Reshape to spatial format: [bs, feat_h, feat_w, embed_dim] -> [bs, embed_dim, feat_h, feat_w]
      x = x.view(bs, feat_h, feat_w, -1).permute(0, 3, 1, 2)
    else:
      # Handle CNN output which is 4D [bs, channels, height, width]
  ...
  def dense_trunk_head(self, x):
    # Handle case where head is Identity (like in BiomedCLIP)
    if isinstance(self.trunk.head, nn.Identity):
      # For Identity head, just permute dimensions
      x = x.permute(0, 2, 3, 1)
    else:
      # For other heads with norm and drop
  ...
  def _extract_roi_features_v2(self, x, normed_boxes, **kwargs):
    ...
    roi_feats = roi_align(x, self._denormalize_boxes(normed_boxes, x), (1, 1), 1.0, -1, True)[..., 0, 0]
    return roi_feats
  ...
  def encode_rois_and_image(self, x, normed_boxes, **kwargs):
    ...
    tar_w = (self.image_size[1] * w_f) // w
    x_image = x
    x_rois = roi_align(x, self._denormalize_boxes(normed_boxes, x), (tar_h, tar_w), 1.0, -1, True)
