...
def create_model(
    ...
    if has_hf_hub_prefix:
        ...
        ################################################
        if model_cfg["text_cfg"]:
            del model_cfg["text_cfg"]["hf_proj_type"]
            del model_cfg["text_cfg"]["hf_pooler_type"]
        ################################################
    else:
...
def create_model_and_transforms(
    ...
    if dataset_type == "sanity_check":
        ...
        preprocess_train = (
            [preprocess_val_det, preprocess_val_img]
            if "distill" in dataset_type
            or dataset_type == "region_clip"
...
def demo_hf_model():
    ...
    model, preprocess_train, preprocess_val = create_model_and_transforms(
        ...
        pretrained=None,  # or None if you want to skip weights
        precision="fp32",
        device="cuda",
        output_dict=True,
        dataset_type="clipself",
    )
    print("Model created:", type(model))
    print("Preprocess train:", preprocess_train)
    print("Preprocess val:", preprocess_val)
