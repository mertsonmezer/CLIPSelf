...
class CLIP(nn.Module):
  ...
  def _pool_masks_v3(self, image, masks, normalize):
    ...
    x_dense = torch.repeat_interleave(x_dense, torch.tensor([len(m) for m in masks], device=x_dense.device), dim=0)
...
def trace_model(model, batch_size=256, device=torch.device("cpu")):
  ...
  model = torch.jit.trace_module(
    model,
    inputs=dict(forward=(example_images, example_text), encode_text=(example_text,), encode_image=(example_images,)),
  )
