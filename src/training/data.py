...
class ProposalDistillDataset(Dataset):
  def __init__(self, input_filename, transforms, image_root, crop_size=224, tokenizer=None, args=None):
...
class COCOPanopticDataset(Dataset):
  def __init__(
    ...
    )  # downsample to the output size of image encoder
  ...
  def __getitem__(self, idx):
    ...
    for i, ann in enumerate(anns):
      ...
      np_old_image = np_old_image.copy()  # Make the image writable
